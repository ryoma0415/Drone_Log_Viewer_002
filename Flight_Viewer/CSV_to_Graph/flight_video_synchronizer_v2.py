...
DEFAULT_TARGET_POSITION = (0.325, -0.325)  # 必要に応じて目標位置を更新

class FlightVideoSynchronizer:
    def __init__(self, csv_path, video_path, output_path=None, target_position=(0.0, 0.0)):
        """
        初期化
        ...
        Args:
            ...
            target_position: 目標位置の(X, Y)座標（m）
        """
        ...
        self.colors = {
            ...
            'feedback_roll': '#F1C40F',  # フィードバックRoll
            'feedback_pitch': '#9B59B6',  # フィードバックPitch
            'rb_marker': '#2ECC71'  # 剛体マーカー数
        }
        self.marker_axis_max = None
    ...
    def create_figure(self):
        ...
        ax_feedback = fig.add_subplot(gs[1, 3])
        ax_feedback.set_title('StampFly目標姿勢 [deg]', fontsize=12, color='white')
        ax_feedback.set_xlabel('時間 [s]', fontsize=10, color='white')
        ax_feedback.set_ylabel('角度 [deg]', fontsize=10, color='white')
        ax_feedback.grid(True, alpha=0.3, color='gray')
        ax_feedback.set_facecolor('#2e2e2e')

        ax_pid_x = fig.add_subplot(gs[2, 0])
        ax_pid_x.set_title('X軸PID成分', fontsize=12, color='white')
        ...
        ax_pid_y = fig.add_subplot(gs[2, 1])
        ax_pid_y.set_title('Y軸PID成分', fontsize=12, color='white')
        ...
        ax_markers = fig.add_subplot(gs[2, 2:])
        ...
        for ax in [ax_2d, ax_angle, ax_feedback, ax_pid_x, ax_pid_y, ax_markers]:
    ...
    def interpolate_csv_to_video_fps(self):
        ...
        if 'feedback_roll_rad' in interpolated_data.columns:
            interpolated_data['feedback_roll_deg'] = np.degrees(interpolated_data['feedback_roll_rad'])
        if 'feedback_pitch_rad' in interpolated_data.columns:
            interpolated_data['feedback_pitch_deg'] = np.degrees(interpolated_data['feedback_pitch_rad'])
    ...
    def animate(self, frame_idx, axes, interpolated_df, video_frames):
        ...
        pos_x_col = 'raw_pos_x' if 'raw_pos_x' in interpolated_df.columns else 'pos_x'
        pos_y_col = 'raw_pos_y' if 'raw_pos_y' in interpolated_df.columns else 'pos_y'
        ...
        if len(history_data) > 1:
            traj_x = history_data[pos_x_col].to_numpy()
            traj_y = history_data[pos_y_col].to_numpy()
            for i in range(len(traj_x) - 1):
                alpha = (i + 1) / len(traj_x) * 0.7
                axes['2d'].plot(traj_x[i:i+2], traj_y[i:i+2],
                                color=self.colors['trajectory'], alpha=alpha, linewidth=2)

        axes['2d'].scatter(current_data[pos_x_col], current_data[pos_y_col],
                          color=self.colors['current'], s=100, zorder=5,
                          edgecolors='white', linewidth=2, label='現在位置')
        target_x, target_y = self.target_position
        axes['2d'].scatter(target_x, target_y, color='yellow', marker='x', s=100,
                          linewidth=3, label='目標位置', zorder=4)

        axes['2d'].set_xlim(target_x - 0.5, target_x + 0.5)
        axes['2d'].set_ylim(target_y - 0.5, target_y + 0.5)
        ...
        axes['stampfly'].clear()
        axes['stampfly'].set_title('StampFly目標姿勢 [deg]', fontsize=12, color='white')
        axes['stampfly'].set_xlabel('時間 [s]', fontsize=10, color='white')
        axes['stampfly'].set_ylabel('角度 [deg]', fontsize=10, color='white')

        if 'feedback_roll_deg' in history_data:
            axes['stampfly'].plot(history_data['elapsed_time'], history_data['feedback_roll_deg'],
                                  color=self.colors['feedback_roll'], label='Roll', linewidth=2, alpha=0.9)
        if 'feedback_pitch_deg' in history_data:
            axes['stampfly'].plot(history_data['elapsed_time'], history_data['feedback_pitch_deg'],
                                  color=self.colors['feedback_pitch'], label='Pitch', linewidth=2, alpha=0.9)

        axes['stampfly'].axhline(y=0, color='gray', linestyle='--', alpha=0.5)
        axes['stampfly'].set_xlim(current_time - 5, current_time + 0.5)
        axes['stampfly'].set_ylim(-5, 5)
        axes['stampfly'].grid(True, alpha=0.3, color='gray')
        axes['stampfly'].legend(loc='upper right', fontsize=8, framealpha=0.8)
        axes['stampfly'].set_facecolor('#2e2e2e')

        axes['pid_x'].clear()
        axes['pid_x'].set_title('X軸PID成分', fontsize=12, color='white')
        ...
        axes['pid_y'].clear()
        axes['pid_y'].set_title('Y軸PID成分', fontsize=12, color='white')
        ...
        if 'marker_count' in history_data:
            axes['markers'].fill_between(history_data['elapsed_time'], 0,
                                         history_data['marker_count'],
                                         color=self.colors['rb_marker'], alpha=0.6)
            axes['markers'].plot(history_data['elapsed_time'], history_data['marker_count'],
                                 color=self.colors['rb_marker'], linewidth=2)

        axes['markers'].axvline(x=current_time, color='red', linestyle='--', alpha=0.7)
        marker_top = self.marker_axis_max if self.marker_axis_max is not None else 5
        if marker_top <= 0:
            marker_top = 1
        marker_value = current_data.get('marker_count', np.nan)
        if pd.notna(marker_value):
            axes['markers'].text(current_time, marker_top * 0.9, f'{int(marker_value)}個',
                                 ha='center', fontsize=14, color='white',
                                 bbox=dict(boxstyle='round', facecolor='red', alpha=0.7))

        axes['markers'].set_xlim(current_time - 10, current_time + 1)
        axes['markers'].set_ylim(0, marker_top)
    ...
    def generate_video(self):
        ...
        if 'marker_count' in interpolated_df.columns:
            marker_max = interpolated_df['marker_count'].max()
            if pd.isna(marker_max):
                marker_max = 5
            self.marker_axis_max = max(5, marker_max + 1)
        else:
            self.marker_axis_max = 5
...
def main():
    ...
    try:
        # ビジュアライゼーション生成
        synchronizer = FlightVideoSynchronizer(csv_path, video_path, target_position=DEFAULT_TARGET_POSITION)
